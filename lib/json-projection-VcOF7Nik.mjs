...
function requireIsoDate$2(value, label, fail) {
	if (!isIsoDate$1(value)) fail("invalid_iso_date", `${label} must be a valid ISO calendar date (YYYY-MM-DD)`);
	return value;
}
function requireSourceDate$2(value, label, fail) {
	...
	if (timestamp === null || !isIsoDate$1(timestamp[1]) || Number(timestamp[2]) > 23 || Number(timestamp[3]) > 59 || Number(timestamp[4]) > 59) fail("invalid_iso_date", `${label} must be an ISO calendar date or UTC RFC 3339 timestamp`);
	return timestamp[1];
}
	...
	scanString() {
		...
		while (this.cursor < this.input.length) {
			...
			if (character === "\"") {
				...
				if (hasUnpairedSurrogate$2(decoded)) this.policy.fail("invalid_unicode", "JSON strings must not contain unpaired UTF-16 surrogates");
				return decoded;
			}
...
function decodeJsonInput(input, policy) {
	if (typeof input === "string") {
		if (hasUnpairedSurrogate$2(input)) policy.fail("invalid_unicode", "JSON input must not contain unpaired UTF-16 surrogates");
...
function scanStrictJson(text, policy) {
	new StrictJsonScanner$2(text, policy).scan();
}
...
//# sourceMappingURL=json-projection-VcOF7Nik.mjs.map