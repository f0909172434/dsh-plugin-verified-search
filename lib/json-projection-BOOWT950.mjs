	...
	scanString() {
		...
		while (this.cursor < this.input.length) {
			...
			if (character === "\"") {
				...
				if (hasUnpairedSurrogate(decoded)) this.policy.fail("invalid_unicode", "JSON strings must not contain unpaired UTF-16 surrogates");
				return decoded;
			}
...
function decodeJsonInput(input, policy) {
	if (typeof input === "string") {
		if (hasUnpairedSurrogate(input)) policy.fail("invalid_unicode", "JSON input must not contain unpaired UTF-16 surrogates");
...
function failJsonPrimitive$1(kind, message, options) {
...
function parsePointer$2(pointer, label) {
	return parseJsonPointer(pointer, label, {
		...
		fail: failJsonPrimitive$1
	});
}
function requireIsoDate$1(value, label) {
	return requireIsoDate$2(value, label, failJsonPrimitive$1);
}
function requireSourceDate$1(value, label) {
	return requireSourceDate$2(value, label, failJsonPrimitive$1);
}
...
const JSON_PRIMITIVE_FAILURE_CODES = {
	invalid_request: "JSON_PROJECTION_INVALID_REQUEST",
	input_too_large: "JSON_PROJECTION_INPUT_TOO_LARGE",
	invalid_utf8: "JSON_PROJECTION_INVALID_UTF8",
	invalid_unicode: "JSON_PROJECTION_INVALID_UNICODE",
	invalid_json: "JSON_PROJECTION_INVALID_JSON",
	duplicate_key: "JSON_PROJECTION_DUPLICATE_KEY",
	parse_limit_exceeded: "JSON_PROJECTION_PARSE_LIMIT_EXCEEDED",
	invalid_pointer: "JSON_PROJECTION_INVALID_POINTER",
	invalid_iso_date: "JSON_PROJECTION_INVALID_REQUEST"
};
function failJsonPrimitive(kind, message, options) {
	return fail(message, JSON_PRIMITIVE_FAILURE_CODES[kind], options);
}
...
//# sourceMappingURL=json-projection-BOOWT950.mjs.map