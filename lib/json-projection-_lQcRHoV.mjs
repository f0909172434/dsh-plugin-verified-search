...
function parseStrictJson$2(text, policy) {
...
const JSON_PRIMITIVE_ERROR_CODES$1 = {
...
function failJsonPrimitive$1(kind, message, options) {
	fail$2(message, JSON_PRIMITIVE_ERROR_CODES$1[kind], options);
}
...
function parseLosslessJson(text, policy) {
	scanStrictJson(text, {
		maxDepth: policy.maxDepth,
		fail: policy.primitiveFail
	});
	let numberTokens = 0;
	try {
		...
		if (error instanceof SyntaxError) policy.fail("invalid_json", "JSON input is invalid", { cause: error });
		throw error;
	}
}
...
function failJsonPrimitive(kind, message, options) {
	fail$1(message, JSON_PRIMITIVE_ERROR_CODES[kind], options);
}
const LOSSLESS_NUMBER_ERROR_CODES = {
	invalid_json: "JSON_NUMERIC_SELECTION_INVALID_JSON",
...
function failLosslessNumber(kind, message, options) {
	fail$1(message, LOSSLESS_NUMBER_ERROR_CODES[kind], options);
}
...
function selectJsonNumericTies(input, rawRequest) {
	...
	const evidenceSha256 = createHash("sha256").update(decoded.bytes).digest("hex");
	const root = parseLosslessJson(decoded.text, {
		...
		primitiveFail: failJsonPrimitive,
		fail: failLosslessNumber
	});
...
//# sourceMappingURL=json-projection-_lQcRHoV.mjs.map