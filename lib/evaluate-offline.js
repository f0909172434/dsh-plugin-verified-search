#!/usr/bin/env node
import { d as normalizeAllowedDomains, f as sourceMatchesDomain, i as selectJsonNumericTies, n as projectJsonRows, o as selectJsonMaxTies, u as filterAllowedSources } from "./json-projection-VcOF7Nik.mjs";
...
function isOfflineJsonObject(value) {
	return typeof value === "object" && value !== null && !Array.isArray(value);
}
...
function requireInteger(value, label, minimum = 0) {
	if (typeof value !== "number" || !Number.isSafeInteger(value) || value < minimum) contractFail(`${label} must be a safe integer >= ${minimum}`);
	return value;
}
	...
	if (record.status === "pass") {
		...
		return {
			...
			assertions: record.assertions.map((raw, index) => {
				...
				if (typeof assertion.pointer !== "string") contractFail(`${assertionLabel}.pointer must be a string`);
				if (assertion.pointer !== "" && !assertion.pointer.startsWith("/")) contractFail(`${assertionLabel}.pointer must be an RFC 6901 JSON Pointer`);
				return {
...
function parseOfflineEvaluationManifest(value) {
	...
	const suites = record.suites.map((raw, index) => {
		...
		if (file.includes("/") || file.includes("\\") || file === "." || file === "..") contractFail(`${label}.file must be a basename`);
		const sha256 = requireString(suite.sha256, `${label}.sha256`);
		if (!/^sha256:[0-9a-f]{64}$/u.test(sha256)) contractFail(`${label}.sha256 must be a lowercase SHA-256 digest`);
		return {
...
function decodeSuiteBytes(bytes, file) {
	let text;
	try {
		text = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
	} catch (error) {
		throw new OfflineEvaluationContractError(`suite ${file} is not valid UTF-8: ${error instanceof Error ? error.message : String(error)}`);
	}
	let value;
	try {
		value = JSON.parse(text);
	} catch (error) {
		throw new OfflineEvaluationContractError(`suite ${file} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
	}
...
function verifyOfflineEvaluationBundle(manifest, loadedSuites) {
	...
	for (const expected of manifest.suites) {
		...
		if (canonicalJson(requireJsonValue(parsedFromBytes, expected.file)) !== canonicalJson(requireJsonValue(loaded.suite, `${expected.file} parsed suite`))) contractFail(`loaded suite object does not match exact bytes: ${expected.file}`);
		if (parsedFromBytes.suiteId !== expected.suiteId) contractFail(`suite ID mismatch for ${expected.file}`);
		if (parsedFromBytes.capability !== expected.capability) contractFail(`suite capability mismatch for ${expected.file}`);
		if (parsedFromBytes.cases.length !== expected.caseCount) contractFail(`suite case count mismatch for ${expected.file}`);
...
function resolveAssertionPointer(root, pointer) {
	...
	for (const segment of pointerSegments(pointer)) {
		...
		if (!isOfflineJsonObject(value) || !Object.prototype.hasOwnProperty.call(value, segment)) contractFail(`assertion pointer was not found: ${pointer}`);
		value = value[segment];
	}
	return value;
}
