...
This is an outbound-retrieval safety boundary, not a trust judgment about page content. Exact fetched excerpts, projected JSON scalar values, source/final URLs, retrieval timestamps, selection metadata, and content hashes can become durable tool/session data and are explicitly presented to the model as untrusted. Do not search for private URLs or content, project private fields, or put secrets in query parameters; URL sanitization is defensive and cannot identify every application-specific credential name. JSON hashes authenticate only the decoded bytes used by the selector, not the publisher or the truth of the feed.
